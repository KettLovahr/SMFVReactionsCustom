...
*
*	function th remove the casted reaction previosuly.
*   removeReaction($msg_id, $emoji_id, $member_id) 
*
...
function addreaction($msg_id, $emoji_id, $member_id) {
	...
	global $context, $db_prefix, $smcFunc, $modSettings, $txt;

	//load the language for strings..
	loadLanguage('vreactions');

	//is it really a member? or a guest/unknown?
	if(!$member_id || $member_id == 0 || $context['user']['is_guest']) fatal_error($txt['vreactions_error_not_reg'], false);
	...
	for ($x=0; $x<sizeof($who_reacts); $x++) {
		...
		if($member_id == $who_reacts[$x]) {
			if(!empty($modSettings['vreactions_revert_enabled'])) removeReaction($msg_id, $emoji_id, $member_id);
			else fatal_error($txt['vreactions_error_already_reacted'], false);
...
//this is to remove the reaction of member who has previously reacted.
function removeReaction($msg_id, $emoji_id, $member_id) {
	...
	//checking if there are multiple reactions to the selected emoji.
	$row = $smcFunc['db_fetch_assoc']($query);

	if( $row['no_of_reacts'] > 1 ) {
		...
		//now, we wanna reduce no of reactions and also remove the user id in database.
		$request = $smcFunc['db_query']('',
			'UPDATE {db_prefix}v_reactions 
			SET members_id = {string:id_member}, no_of_reacts = no_of_reacts-1 
			WHERE msg_id = {int:id_msg} 
			AND emoji_id = {int:id_emoji}',
			array(
				'id_member' => $who_reacts,
				'id_msg' => $msg_id,
				'id_emoji' => $emoji_id,
			)
		);
		...
		//announce if successfull/failed.
		if ($request) return $txt['vreactions_unreacted'];
		else return $txt['vreactions_failed'];
	}
	//otherwise it means there's only 1 reaction. We will delete the entry.
	else {
		...
		return $txt['vreactions_unreacted'];	
	}
